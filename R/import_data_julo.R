...
import_data_julo <- function(analytics_path = system.file(file.path("shiny",
  ...
  operation_mySQL_conf = system.file("shiny/julo/.my.cnf",
                                      package = "aquanes.report"),
  operation_meta_path = system.file(file.path("shiny/julo/data",
...
analytics_4015 <- import_sheets(xlsPath = analytics_path,
                                sheets_analytics = analytics_to_import,
                                skip = skip)


drop.cols <- c("Who", "Comments", "LocationName", "LocationID")
select.cols <- dplyr::setdiff(names(analytics_4015),drop.cols)

analytics_4015   <- analytics_4015[,select.cols] %>%
