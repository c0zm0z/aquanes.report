...
```{r, echo=FALSE}
   ...
   online <- config$report_data[config$report_data[,"ParameterName"] %in% config$report_parameters_online,] %>% 
    ...
    g1 <- ggplot2::ggplot(online, ggplot2::aes_string(x = "DateTime",
     ...
     if (config$report_add_thresholds == TRUE & config$report_parameters_online[i] %in% unique(thresholds$ParameterName)) {
     
     sel_threshold1 <- thresholds[thresholds$ParameterName == config$report_parameters_online[i],]
...
```{r, echo=FALSE}
   ...
   offline <- config$report_data[config$report_data[,"ParameterName"] %in% config$report_parameters_offline,] %>% 
    ...
    if (config$report_add_thresholds == TRUE & config$report_parameters_offline[i] %in% unique(thresholds$ParameterName)) {
     sel_threshold2 <- thresholds[thresholds$ParameterName == config$report_parameters_offline[i],]
